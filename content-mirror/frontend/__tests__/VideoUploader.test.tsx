...
describe("VideoUploader", () => {
  describe("idle state", () => {
    it("renders the drop zone with descriptive text", () => {
      render(<VideoUploader onUpload={noop} isUploading={false} progress={0} />);
      expect(screen.getByText(/drag & drop your video/i)).toBeInTheDocument();
    ...
    it("Analyze URL button is disabled (Phase 2)", () => {
      render(<VideoUploader onUpload={noop} isUploading={false} progress={0} />);
    ...
    it("updates URL input value as user types", async () => {
      render(<VideoUploader onUpload={noop} isUploading={false} progress={0} />);
      const input = screen.getByPlaceholderText(/tiktok\.com/i);
  ...
  describe("uploading state", () => {
    it("shows a spinner instead of the drop zone", () => {
      render(<VideoUploader onUpload={noop} isUploading progress={40} stage="analyzing_audio" />);
      expect(screen.queryByText(/drag & drop/i)).not.toBeInTheDocument();
    });

    it("displays the correct stage label", () => {
      render(<VideoUploader onUpload={noop} isUploading progress={55} stage="analyzing_audio" />);
      expect(screen.getByText("Analyzing audio quality...")).toBeInTheDocument();
    });

    it("shows fallback label for unknown stage", () => {
      render(<VideoUploader onUpload={noop} isUploading progress={20} />);
      expect(screen.getByText("Analyzing your content...")).toBeInTheDocument();
    });

    it("shows correct progress percentage", () => {
      render(<VideoUploader onUpload={noop} isUploading progress={73} stage="generating_insights" />);
    ...
    it("shows each stage label correctly", () => {
      ...
      for (const [stage, label] of stages) {
        const { unmount } = render(
          <VideoUploader onUpload={noop} isUploading progress={50} stage={stage as never} />
        );
